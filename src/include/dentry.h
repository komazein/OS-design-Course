...
public:
    ...
    /**
     * 
     * @brief 检查是否拥有子, 首先查找全局hash, 再根据DFLAG判断是否需要I/O来确定是否存在子
     * 在进行删除目录的时候, 需要级联删除, 
     * 即如果rmdir dir后, 需要删除以dir为根的目录, 同时也要删除磁盘中的相关所有目录项,
     * 此操作可能涉及I/O操作, 所以不能使用dentry来实现功能
     * 
     * 
     * 
     ...
     * 
     */
    void cut_tree(dentry* dentry_node, size_t& counter, bool is_del);
     ...
     * 
     */
    size_t cut_dirTree(double cut_ratio, dentry* work_dir);
