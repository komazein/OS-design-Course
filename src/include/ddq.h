...
public:
    ...
    bool freeinode(size_t ino);
    void load(dirTree*dir_tree_);
    void freeSIMFILE(inode&ino,size_t primsizeofchild,inode&par);
    string getUSERroot(string username,string passwoard,int&uid,int&gid);
    void freshUSER();
    size_t getfreeinodenum();
    void Wr_without(size_t inum);
    void freeIN(size_t ino);
    void INSTER(inode*ino);
private:
    super_block* sb;
    dirTree* dirtree_;
