...
void user::authorize(dentry* accessed_file, int authorized_user_id)
{
    ...
    else
    {
        ...
        accessed_file->get_inode()->i_acl=acl;
        cout<<(int)(accessed_file->get_inode()->i_acl.owner)<<endl;
        accessed_file->set_dirty(true);
...
void user::fomat()
{   
    string user_name2 = "user" + std::to_string(uid_);
    dir_tree_->free_dir(user_name2, dir_tree_->get_root());
    dir_tree_->alloc_dir(user_name2,dir_tree_->get_root(),nullptr,DIR);
    //缁撴潫鍚庝护褰撳墠鑺傜偣绛変簬鏍硅妭鐐?
}
