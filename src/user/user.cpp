...
void user::authorize(std::string& accessed_file, int authorized_user_id)
{
    auto cur_dir = fileSystem->getCurrentDir();

    auto dentry_node = dir_tree_->name_travesal(accessed_file, cur_dir);

    if (dentry_node == nullptr) {
        spdlog::warn("File '{}' not found in current directory '{}'", accessed_file, cur_dir->get_name());
        return;
    }
    ...
    if((acl.owner >> uid) & 1)
    {
        spdlog::warn("User {} has already been the owner of the file {}", uid, dentry_node->get_name());
    }
    else
    {
        // if the user is not the owner, set the owner bit 1 which represents the user has been the owner of the file
        acl.owner |= (1 << uid);
        spdlog::info("User {} is now authorized to access the file {}", uid, dentry_node->get_name());
    }
}
...
void user::recoverAuthorization(std::string& accessed_file, int authorized_user_id)
{
    ...
    if(!(acl.owner >> uid) & 1)
    {
        spdlog::info("User {} is not authorized to access the file {}, no need to recovery", uid, dentry_node->get_name());
    }
    else
    {
