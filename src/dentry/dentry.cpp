...
dentry* dirTree::name_travesal(string& path, dentry* work_dir)
{
    ...
    while(getline(ss, name, '/')){
        ...
        if(dentry_next == nullptr){
            ...
            if(dentry_next == nullptr) {
                if(search->get_flag() == FIRST_LOAD_TO_MEMORY) {
                    ...
                    search->set_flag(FIRST_LOAD_TO_MEMORY);
        ...
        cout << "+++++)))))))))" << search->get_name() << endl;

        // 此时添加进入lru_list, 因为此时算上一次最新的访问
        dentry_replacer_->InsertDir(dentry_next);
    ...
    auto lru_list = dentry_replacer_->get_list();
    cout << "############################\n";
        for(auto node : lru_list){
            cout << node->get_name() << "   ";
        }
        cout << endl;
...
bool dirTree::alloc_dir(string& name, dentry* work_dir,inode* new_allocate_inode, TYPE type)
{
    ...
    cout<<"new"<<new_allocate_inode->i_num<<" "<<new_allocate_inode->di_link_count<<endl;
    ...
    new_allocate_inode->i_uid=logInUser->getUid();
    new_allocate_inode->i_gid=logInUser->getGid();
    cout<<"new"<<new_allocate_inode->i_num<<" "<<new_allocate_inode->di_link_count<<endl;
    ///  .....
    bool ableGetBlock= bs->creatFILE(work_dir->get_subdir().size(),*work_dir->get_inode(),*new_allocate_inode);
    ...
    if(ableGetBlock==false)
    {
        bs->freeIN(new_allocate_inode->i_num);
        spdlog::warn("NO FREE BLOCK");
        return false;///块不够
    }
    get_bs()->INSTER(new_allocate_inode);
...
bool dirTree::alloc_dir_init(string& name, dentry* work_dir,inode* new_allocate_inode, TYPE type,size_t uid)
{
    ...
    new_allocate_inode->i_uid =uid;
    new_allocate_inode->i_gid=((uid-1)>>1)+1;
...
void dirTree::del_tree(dentry* dentry_root,vector<pair<inode ,size_t>>&del_nodes)
{
    ...
    // cache_->erase_dentry(dentry_root->get_name(), dentry_root->get_parent());
    delete dentry_root;
...
void dirTree::cut_tree(dentry* dentry_node, size_t& counter,bool is_del)
{
    ...
    if(!dentry_node) { return; }        //仅为安全性检查, 正常不会执行
        ...
        if(dentry_node->get_dirty()){
            ...
            bs->Wr_without(dentry_node->get_inode_num());
        ...
        for (auto& [name, child_node] : sub_dirs) {
            ...
            cut_tree(child_node, counter,true);           // 释放节点
        }
    ...
    if(is_del) 
    {
        cache_->erase_dentry(dentry_node->get_name(), dentry_node->get_parent());
        delete dentry_node;
        ++counter;// 统计删除的个数  
    }
    else
    {
        dentry_node->set_flag(UNLOAD_CHILD_FROM_DISK);
        dentry_node->set_dirty(false);
    }
    return;
}
...
size_t dirTree::cut_dirTree(double cut_ratio, dentry* work_dir)
{
    size_t counter = 0;
    ...
    if(cut_ratio != 0){
        ...
        // 获得需要剪枝的个数
        size_t shrink_num = static_cast<size_t>(ceil(dentry_replacer_->get_max_size() * cut_ratio));

        cout << shrink_num << "((((((((((((((((((()))))))))))))))))))\n";
        cout << dentry_replacer_->get_cur_size() << endl;
        auto lru_list = dentry_replacer_->get_list();
        for(auto node : lru_list){
            cout << node->get_name() << "   ";
        }
        ...
        while(counter < shrink_num && dentry_replacer_->get_cur_size() != 0){
            ...
            if(dentry_replacer_->get_cur_size() == 1){
                break;
            }
            size_t cur_level_counter = 0;

            auto vitim_opt = dentry_replacer_->Victim();        // 选出最久未使用的dentry节点, **释放其子**


            cout << work_dir->get_name() << "+++++++++++"<< endl;
            cout << vitim_opt.value()->get_name() << "+++++++++++"<< endl;


            if(work_dir == vitim_opt.value() || work_dir->get_parent() == vitim_opt.value())
            {
                cout << work_dir->get_name() << "+++++++++++"<< endl;
                dentry_replacer_->Insert(vitim_opt.value());
                break;
            }
            ...
            // has_child_test(vitimnode);
            auto child_nodes = vitimnode->get_subdir();

            cut_tree(vitimnode, cur_level_counter,false);

            // for(auto& [_, node] : child_nodes){      // 释放其子节点

            //     cout << "cut node : " << node->get_name() << endl;

            //     cut_tree(node, cur_level_counter);
            // }

            vitimnode->clear_child();               ///////////////////// 释放所有子

            // exit(1);
            
            if(cur_level_counter){        // 如果释放了其子节点, 则标记为此节点被剪枝
                vitimnode->set_flag(UNLOAD_CHILD_FROM_DISK);
            }
            counter += cur_level_counter;
        }
    ...
    //     dentry* vitimnode = vitim_opt.value();
    //     cut_tree(vitimnode, counter,false);
    //     // auto child_nodes = vitimnode->get_subdir();
