...
void blockScheduler::freeblock(vector<pair<inode, size_t>>&del_nodes,inode&par,size_t primsizeofchild)
{
    ...
    {
        bool y=freeinode(del_nodes[i].first.i_num);////////当前节点可释放
        if(y==false)
            continue;
        cout<<"---------------"<<del_nodes[i].first.i_num<<endl;
...
void blockScheduler::freeSIMFILE(inode&ino,size_t primsizeofchild,inode&par)/////////不用了
{
    vector<size_t>need;
    vector<size_t>emptyvec;
    getallBlockSIM(ino,need,emptyvec);
    bool y=freeinode(ino.i_num);
    if(y==false)
        return;
    if(cal_block_num_dir(primsizeofchild)!=cal_block_num_dir(primsizeofchild-1))
        need.push_back(getlastblockID(primsizeofchild,par));
...
void blockScheduler::ReWrinode(inode &ino,bool read)
{
    ...
    if(read)
    {
        ...
        else
        {
            ...
            inode_hash.emplace(ino.i_num,std::make_pair(&ino,(size_t)1));
        }
    }
    else
    {
        ...
        if(it!=inode_hash.end())
        {
            cout<<"HASH::NOTFIND::ino_inum"<<ino.i_num<<" "<<read<<" "<<endl;
            (*it).second.second--;
            if((*it).second.second==0)
            {
                cout<<"HASH:::ino_inum"<<ino.i_num<<" "<<ino.di_link_count<<" "<<endl;
...
void blockScheduler::INSTER(inode*ino)
{
    inode_hash.emplace(ino->i_num,std::make_pair(ino,(size_t)1));
}
void blockScheduler::Wr_without(size_t inum)
{
    auto it=inode_hash.find(inum);
    if(it!=inode_hash.end())
    {
        (*it).second.second--;
        if((*it).second.second==0)
...
bool blockScheduler::freeinode(size_t ino)
{
    auto it=inode_hash.find(ino);
    if(it==inode_hash.end())//未找到,在磁盘中
    {
        inode*temp=(inode*)malloc(sizeof(inode));
        temp->i_num=ino;
        ReWrinode(*temp,true);//////////先读出来
        if(temp->di_link_count>0)
        {
            cout<<"di_count:d:"<<temp->di_link_count<<" "<<ino<<endl;
            temp->di_link_count--;//硬链接计数减一
            ReWrinode(*temp,false);////写回去
            return false;
        }
        else if(temp->di_link_count==0)
        {
            Wr_without(ino);
            cout<<"di_count:c:"<<temp->di_link_count<<" "<<ino<<endl;
            sb->freeinode(ino);//不存在硬链接，释放
            return true;
        }
    }
    else
    {
        if((*it).second.first->di_link_count>0)
        {
            cout<<"di_count:a:"<<(*it).second.first->di_link_count<<" "<<ino<<endl;
            (*it).second.first->di_link_count--;//计数发生了改变
            ReWrinode(*(*it).second.first,false); //写回
            return false;
        }
        else if((*it).second.first->di_link_count==0)
        {
            Wr_without(ino);
            cout<<"di_count:s:"<<(*it).second.first->di_link_count<<" "<<ino<<endl;
            sb->freeinode(ino);
            return true;
        }
    }
...
void blockScheduler::freeIN(size_t ino)
{
    sb->freeinode(ino);
}
